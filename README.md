...
Vendor-first middleware backend for TiffinOS/TifinOS built with NestJS, Prisma, PostgreSQL, Redis, and OTP-based authentication.

## Vendor APIs

The backend now includes vendor-first onboarding and authentication endpoints that cover:

- Indian mobile registration and login with OTP
- 30-second resend cooldown and 5-minute OTP expiry
- Vendor onboarding with business name, locality, multiple service areas, meals offered, and UPI ID
- Unique 10-character invite code generation
- Invite link, WhatsApp share link, and QR metadata for vendor acquisition
- Persistent JWT session via bearer token and auth cookie
