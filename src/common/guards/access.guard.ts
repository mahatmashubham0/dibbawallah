...
import { AdminStatus, Prisma, UserStatus } from '@prisma/client';
...
export class AccessGuard implements CanActivate {
  ...
  async validate(user: AuthenticatedUser) {
    ...
    } else if (user.type === UserType.Vendor) {
      const vendorRows = await this.prisma.$queryRaw<Array<{ status: string }>>(
        Prisma.sql`SELECT status::text AS status FROM vendor WHERE id = ${user.id} LIMIT 1`,
      );
      if (vendorRows[0]?.status !== ACTIVE_VENDOR_STATUS) {
        await this.cacheManager.set(cacheKey, false, cacheTtl);
        throw new UnauthorizedException();
      }
    }
