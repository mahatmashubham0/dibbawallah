...
export class OtpService {
  ...
  private generateCode(length: number): string {
    if (!this.utilsService.isProductionApp()) {
      return this.config.default.slice(0, length).padEnd(length, '0');
    }
  ...
  private getContextMailParams(args: {
    ...
    const data = {
      ...
      code: args.code,
      expirationTime: this.utilsService.msToHuman(args.expiration),
    };
  ...
  private getContextSmsParams(args: {
    ...
    switch (args.context) {
      case OtpContext.Register:
        return {
          text: `Your one time verification code is ${args.code} to register, Please do not share this code to anyone. This verification code will be expired after ${this.utilsService.msToHuman(args.expiration)}`,
        };
      case OtpContext.ResetPassword:
        return {
          text: `Your one time verification code is ${args.code} to reset password, Please do not share this code to anyone. This verification code will be expired after ${this.utilsService.msToHuman(args.expiration)}`,
        };
      case OtpContext.VendorAuth:
        return {
          text: `Your TifinOS vendor verification code is ${args.code}. It expires in ${this.utilsService.msToHuman(args.expiration)}. Do not share this code with anyone.`,
        };
      default:
        throw new Error('Unknown otp context found');
    }
  ...
  async send(
    ...
    const config = {
      ...
      resendCooldown: overrides?.resendCooldown || this.config.resendCooldown,
      expiration: overrides?.expiration || this.config.expiration,
    ...
    if (!otp) {
      ...
      if (
        !this.isTimeout(otp.lastSentAt, config.resendCooldown) &&
        !otp.lastCodeVerified
      ) {
        throw new Error(
          `Resend verification code on ${
            args.target
          } not allowed with in ${this.utilsService.msToHuman(config.resendCooldown)}`,
        );
      }
  ...
  async verify(
    ...
    if (this.isTimeout(otp.lastSentAt, config.expiration)) {
